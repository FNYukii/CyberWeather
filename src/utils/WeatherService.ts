...
class WeatherService {
  static async readCityWeather(
    city: 'osaka' | 'nagoya' | 'tokyo'
  ): Promise<{ weatherCode: number; temp: number; humi: number } | null> {
    // 各都市用のAPIのURL
    const osakaUrl =
      'https://api.open-meteo.com/v1/forecast?latitude=34.6937&longitude=135.5022&hourly=temperature_2m,relative_humidity_2m,weather_code&timezone=Asia%2FTokyo&forecast_days=1'
    const nagoyaUrl =
      'https://api.open-meteo.com/v1/forecast?latitude=35.1815&longitude=136.9064&hourly=temperature_2m,relative_humidity_2m,weather_code&timezone=Asia%2FTokyo&forecast_days=1'
    const tokyoUrl =
      'https://api.open-meteo.com/v1/forecast?latitude=35.6895&longitude=139.6917&hourly=temperature_2m,relative_humidity_2m,weather_code&timezone=Asia%2FTokyo&forecast_days=1'
  ...
  static weatherLabelFromCode(
    code: number
  ): 'sunny' | 'cloudy' | 'fog' | 'rainy' | 'snowy' | 'thunder' | 'unknown' {
    let label:
      | 'sunny'
      | 'cloudy'
      | 'fog'
