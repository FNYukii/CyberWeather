...
function TopScreen() {
  ...
  return (
    <div className="w-screen lg:h-screen   py-24 lg:py-36   flex justify-center items-center">
      {!isLoaded && <Shuffle className="text-3xl">LOADING</Shuffle>}
      ...
      {isLoaded && weatherInfo !== null && (
        <div className="w-full px-4   lg:w-[1024px] lg:px-0">
          <div className="text-center text-3xl sm:text-5xl">
            ...
            <Shuffle className="mt-4" extraFrames={16}>
              {timeText}
            </Shuffle>
          </div>
          ...
          <div className="mt-16   grid grid-cols-1 lg:grid-cols-3   gap-y-12">
            <WeatherCard
              area="osaka"
              weatherCode={weatherInfo.osakaWeatherCode}
              temp={weatherInfo.osakaTemp}
              humi={weatherInfo.osakaHumi}
              className="max-w-96 mx-auto"
            />
            <WeatherCard
              area="nagoya"
              weatherCode={weatherInfo.nagoyaWeatherCode}
              temp={weatherInfo.nagoyaTemp}
              humi={weatherInfo.nagoyaHumi}
              className="max-w-96 mx-auto"
            />
            <WeatherCard
              area="tokyo"
              weatherCode={weatherInfo.tokyoWeatherCode}
              temp={weatherInfo.tokyoTemp}
              humi={weatherInfo.tokyoHumi}
              className="max-w-96 mx-auto"
            />
          </div>
